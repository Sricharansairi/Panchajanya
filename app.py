...
# ── Module imports ────────────────────────────────────────────────────────────
try:
    from ai_engine import generate_curriculum, chat_with_llm, get_backend_status, GROQ_KEYS, GROQ_MODEL
    AI_ENGINE_AVAILABLE = True
except ImportError:
    AI_ENGINE_AVAILABLE = False
    def get_backend_status(): return {"groq_keys": 0, "openrouter_available": False, "any_available": False}
    GROQ_KEYS = []
    GROQ_MODEL = ""

try:
    from pdf_generator import generate_pdf, generate_text_pdf
    PDF_AVAILABLE = True
except ImportError:
    PDF_AVAILABLE = False
...
# CONFIG
# ══════════════════════════════════════════════════════════════════════════════
st.set_page_config(page_title="CurricuForge", page_icon="🎓", layout="wide", initial_sidebar_state="collapsed")
...
# ══════════════════════════════════════════════════════════════════════════════
_DEFAULTS = {
    "current_page": "Home", "curriculum_result": None, "curriculum_error": None,
    "dismiss_warnings": False, "p_skill": "", "p_level": "BTech",
    "p_semesters": 4, "p_hours": 20, "p_industry": "", "p_type": None,
    "p_language": "English", "p_skill_gap": False, "p_current_skills": "",
    "chat_history": [], "curriculum_chat_context": None,
    "compare_result": None, "study_plan": None, "job_mapping": None,
}
...
# WARNINGS
# ══════════════════════════════════════════════════════════════════════════════
_SEM_RANGE = {"Diploma": (2, 4), "BTech": (4, 8), "Master's Degree": (2, 4), "Professional Certification": (2, 3)}
_HR_RANGE  = {"Diploma": (10, 25), "BTech": (15, 30), "Master's Degree": (15, 35), "Professional Certification": (10, 20)}

def _get_warnings(level, semesters, weekly_hours):
    out = []
    slo, shi = _SEM_RANGE.get(level, (2, 8))
    if semesters < slo or semesters > shi:
        out.append(f"**{level}** typically has **{slo}–{shi}** semesters — you chose **{semesters}**.")
    hlo, hhi = _HR_RANGE.get(level, (10, 40))
    if weekly_hours < hlo or weekly_hours > hhi:
        out.append(f"**{level}** typically needs **{hlo}–{hhi} hrs/week** — you entered **{weekly_hours}**.")
    return out
...
def _save_to_history(curriculum):
    ...
    safe = "".join(c if c.isalnum() or c in "_ " else "" for c in title)[:30].strip().replace(" ", "_")
    filename = f"{ts}_{safe}.json"
    filepath = os.path.join(HISTORY_DIR, filename)
    with open(filepath, "w") as f:
        json.dump({"curriculum": curriculum, "saved_at": datetime.now().isoformat(), "title": title}, f, indent=2)
    return filepath
...
def _compute_analytics(cur):
    ...
    stats = {
        "total_courses": 0, "total_credits": 0, "total_topics": 0,
        "sem_names": [], "sem_courses": [], "sem_credits": [], "sem_hours": [],
        "all_topics": [], "course_names": [], "course_credits": [],
    }
...
def _build_curriculum_summary(cur):
    lines = [f"Curriculum: {cur.get('curriculum_title','')}", f"Level: {cur.get('level','')}", ""]
    for sem in cur.get("semesters", []):
        lines.append(f"Semester {sem.get('semester_number','')}: {sem.get('semester_title','')}")
        for c in sem.get("courses", []):
            lines.append(f"  • {c.get('course_code','')} — {c.get('course_name','')}")
            if c.get("topics"):
...
def _render_curriculum(cur, show_export=True):
    ...
    k4.metric("Avg Credits/Sem", round(stats["total_credits"] / max(len(stats["sem_names"]), 1), 1))
    ...
    for sem in cur.get("semesters", []):
        courses = sem.get("courses", [])
        with st.expander(f"Semester {sem.get('semester_number','')}: {sem.get('semester_title','')}  —  {len(courses)} courses", expanded=True):
                ...
                for j, c in enumerate(row):
                    with cols[j]:
                        with st.container(border=True):
                            ...
                            st.write(f"📘 {c.get('credits', '4')} Credits  ·  ⏱️ {c.get('hours_per_week', '3')}h/week")
    ...
    with e1:
        if PDF_AVAILABLE:
            try:
                st.download_button("⬇ PDF Syllabus", generate_pdf(cur), f"{safe}.pdf", "application/pdf", use_container_width=True)
            except Exception as e:
                st.error(f"PDF failed: {e}")
        else:
            st.info("pdf_generator.py not found")
    with e2:
        st.download_button("⬇ JSON Data", json.dumps(cur, indent=2, ensure_ascii=False), f"{safe}.json", "application/json", use_container_width=True)
    with e3:
        if st.button("💬 Ask Doubts", key="ask_doubts", use_container_width=True):
    ...
    with t1:
        if st.button("📅 Generate Study Plan", key="gen_study_plan", use_container_width=True):
            with st.spinner("Creating study plan..."):
                ...
                response, backend = chat_with_llm(prompt, [], "You are a study planning expert. Create detailed, actionable study schedules.")
                st.session_state.study_plan = response

    with t2:
        if st.button("💼 Map to Job Roles", key="gen_job_map", use_container_width=True):
            with st.spinner("Mapping to industry roles..."):
                ...
                response, backend = chat_with_llm(prompt, [], "You are a career counseling expert. Map educational curricula to real industry job roles.")
                st.session_state.job_mapping = response

    with t3:
        if st.button("💾 Save to History", key="save_hist", use_container_width=True):
        ...
        with sp1:
            if PDF_AVAILABLE and generate_text_pdf:
                try:
                    ...
                    st.download_button("⬇ Download Study Plan PDF", sp_pdf, f"{safe}_study_plan.pdf", "application/pdf", key="dl_sp_pdf", use_container_width=True)
                except Exception as e:
                    st.error(f"PDF failed: {e}")
        with sp2:
            if st.button("✕ Close Study Plan", key="close_sp", use_container_width=True):
        ...
        with jm1:
            if PDF_AVAILABLE and generate_text_pdf:
                try:
                    ...
                    st.download_button("⬇ Download Job Mapping PDF", jm_pdf, f"{safe}_job_mapping.pdf", "application/pdf", key="dl_jm_pdf", use_container_width=True)
                except Exception as e:
                    st.error(f"PDF failed: {e}")
        with jm2:
            if st.button("✕ Close Job Mapping", key="close_jm", use_container_width=True):
...
def _page_home():
    ...
    st.write("**CurricuForge** generates complete, semester-wise syllabi in seconds — powered by **Llama 3.3 70B** via Groq + OpenRouter.")

    status = get_backend_status()
    s1, s2, s3, s4 = st.columns(4)
    with s1:
        with st.container(border=True):
            st.markdown(f"{'🟢' if status['groq_available'] else '🔴'} **Groq**")
            st.caption(f"{status['groq_keys']} key{'s' if status['groq_keys'] != 1 else ''}" if status['groq_available'] else "No key")
    with s2:
        with st.container(border=True):
            st.markdown(f"{'🟢' if status['openrouter_available'] else '🟡'} **OpenRouter**")
            st.caption("Fallback ready" if status['openrouter_available'] else "Optional")
    with s3:
        with st.container(border=True):
            st.markdown("📦 **Exports**")
    ...
    for col, title, desc in [(w1, "Step 1 · Configure", "Enter skill, level, semesters, industry."),
    ...
    with col1:
        with st.container(border=True):
            ...
            st.write("**Strong academic foundation** with progressive difficulty.")
            st.markdown("- ✓ Foundation courses first\n- ✓ Progressive difficulty\n- ✓ BTech / Diploma\n- ✓ 4–8 semesters")
            if st.button("🎓  Start as Fresher", key="btn_fresher", use_container_width=True):
                st.session_state.update(current_page="Generator", p_type="fresher", p_level="BTech", p_semesters=4, p_hours=20)
                st.rerun()
    with col2:
        with st.container(border=True):
            st.subheader("💼 Working Professionals")
            st.write("**Less theory**, more industry-focused & hands-on.")
            st.markdown("- ✓ Industry-specific\n- ✓ Hands-on projects\n- ✓ Certifications & Master's\n- ✓ 2–3 semesters")
            if st.button("💼  Start as Professional", key="btn_pro", use_container_width=True):
                st.session_state.update(current_page="Generator", p_type="professional", p_level="Professional Certification", p_semesters=2, p_hours=15)
                st.rerun()
...
def _page_generator():
    ...
    if not status['any_available']:
        ...
        st.code("GROQ_API_KEY=gsk_your_key\nOPENROUTER_API_KEY=sk-or-your_key", language="bash")
        return

    badges = []
    if status['groq_available']:
        badges.append(f"🟢 Groq ({status['groq_keys']} key{'s' if status['groq_keys'] > 1 else ''})")
    ...
    with left:
        ...
        skill = st.text_input("Skill / Subject *", placeholder="e.g. Machine Learning", value=st.session_state.get("p_skill", ""))
        _levels = ["Diploma", "BTech", "Master's Degree", "Professional Certification"]
        _dlevel = st.session_state.get("p_level", "BTech")
        level = st.selectbox("Education Level *", _levels, index=_levels.index(_dlevel) if _dlevel in _levels else 1)
        _sopts = [2, 3, 4, 5, 6, 7, 8]
        _dsem = st.session_state.get("p_semesters", 4)
        semesters = st.selectbox("Semesters *", _sopts, index=_sopts.index(_dsem) if _dsem in _sopts else 2,
                                  format_func=lambda x: f"{x} Semesters ({x // 2} yr{'s' if x // 2 > 1 else ''})")
        weekly_hours = st.number_input("Weekly Hours", min_value=10, max_value=40, value=st.session_state.get("p_hours", 20))
        industry = st.text_input("Industry Focus", placeholder="e.g. AI, FinTech", value=st.session_state.get("p_industry", ""))

        # Language selection
        language = st.selectbox("Output Language", ["English", "Hindi", "Telugu", "Tamil", "Kannada", "Spanish", "French", "German"],
                                 index=0, help="Curriculum will be generated in this language")

        # Skill gap toggle
        st.divider()
        skill_gap_mode = st.toggle("🎯 Skill Gap Analysis", value=False, help="Input your current skills — AI will focus on gaps")
        current_skills = ""
        if skill_gap_mode:
            current_skills = st.text_area("Your current skills", placeholder="e.g. Python basics, HTML/CSS, basic SQL...",
        ...
        b1, b2 = st.columns(2)
        with b1:
            clear_clicked = st.button("🗑️ Clear", key="clr", use_container_width=True)
        with b2:
            generate_clicked = st.button("Generate ✨", key="gen", use_container_width=True)
    ...
    with right:
        if generate_clicked:
            if not skill.strip():
                ...
                payload = {"skill": skill.strip() + hint, "level": level, "semesters": semesters,
                           "weekly_hours": weekly_hours, "industry": industry.strip() or skill.strip()}

                with st.status("Generating curriculum...", expanded=True) as status_widget:
                    st.write("📝 Building prompt...")
                    time.sleep(0.2)
                    be = get_backend_status()
                    st.write(f"🚀 Sending to {'Groq' if be['groq_available'] else 'OpenRouter'} (Llama 3.3 70B)...")
                    if skill_gap_mode:
                    ...
                    try:
                        ...
                        if result.get("success"):
                            ...
                            st.session_state.curriculum_result = result["curriculum"]
                            _save_to_history(result["curriculum"])
                            status_widget.update(label=f"✅ Ready — via {backend}", state="complete", expanded=False)
                        else:
                            st.session_state.curriculum_error = result.get("error", "Unknown error.")
...
def _page_analytics():
    ...
    k5.metric("Avg Topics/Course", round(stats["total_topics"] / max(stats["total_courses"], 1), 1))
    ...
    with c1:
        ...
        fig = go.Figure(go.Bar(
            x=stats["sem_names"], y=stats["sem_courses"],
            marker_color="#6B4EFF", text=stats["sem_courses"], textposition="auto"
        ))
        fig.update_layout(height=350, margin=dict(l=20, r=20, t=20, b=20), plot_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig, use_container_width=True)

    with c2:
        st.subheader("📊 Credits per Semester")
        fig = go.Figure(go.Bar(
            x=stats["sem_names"], y=stats["sem_credits"],
            marker_color="#9B7BFF", text=stats["sem_credits"], textposition="auto"
        ))
        fig.update_layout(height=350, margin=dict(l=20, r=20, t=20, b=20), plot_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with c3:
        ...
        fig = go.Figure(go.Scatter(
            x=stats["sem_names"], y=stats["sem_hours"],
            mode="lines+markers+text", text=stats["sem_hours"], textposition="top center",
            line=dict(color="#6B4EFF", width=3), marker=dict(size=10, color="#6B4EFF")
        ))
        fig.update_layout(height=350, margin=dict(l=20, r=20, t=20, b=20), plot_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with c4:
        ...
        fig = go.Figure(go.Pie(
            labels=stats["sem_names"], values=stats["sem_credits"],
            hole=0.4, marker_colors=px.colors.sequential.Purples_r
        ))
    ...
    st.write("Based on credit load and course hours — higher values suggest more demanding semesters.")
    difficulty = [c + h * 0.5 for c, h in zip(stats["sem_credits"], stats["sem_hours"])]
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=stats["sem_names"], y=difficulty,
        fill="tozeroy", mode="lines+markers",
        line=dict(color="#6B4EFF", width=3),
        marker=dict(size=10), fillcolor="rgba(107,78,255,0.1)"
    ))
...
def _page_history():
    ...
    items = _load_history()
    if not items:
        st.info("📭 **No saved curricula yet.** Generate one in the **Generator** tab — it auto-saves!")
        return
    ...
    for i, item in enumerate(items):
        ...
        courses = sum(len(s.get("courses", [])) for s in cur.get("semesters", []))

        with st.container(border=True):
            h1, h2, h3, h4 = st.columns([3, 1, 1, 1])
            with h1:
                st.markdown(f"**{title}**")
                st.caption(f"🎓 {level}  ·  📅 {sems} semesters  ·  📚 {courses} courses  ·  🕐 {saved_at[:16]}")
            with h2:
...
def _page_compare():
    ...
    st.write("Generate two curricula for different skills and compare them side-by-side.")
    st.divider()

    items = _load_history()
    if len(items) < 2:
        st.info("📭 **Need at least 2 saved curricula.** Generate curricula in the **Generator** tab — they auto-save!")
        return

    titles = [item.get("curriculum", {}).get("curriculum_title", "Untitled") for item in items]

    c1, c2 = st.columns(2)
    with c1:
        idx1 = st.selectbox("Curriculum A", range(len(titles)), format_func=lambda i: titles[i], key="cmp_a")
    with c2:
        idx2 = st.selectbox("Curriculum B", range(len(titles)), format_func=lambda i: titles[i], index=min(1, len(titles)-1), key="cmp_b")
    ...
    st.subheader("📈 Credits per Semester")
    fig = go.Figure()
    fig.add_trace(go.Bar(x=stats_a["sem_names"], y=stats_a["sem_credits"], name=titles[idx1][:20], marker_color="#6B4EFF"))
    fig.add_trace(go.Bar(x=stats_b["sem_names"], y=stats_b["sem_credits"], name=titles[idx2][:20], marker_color="#FF6B4E"))
    fig.update_layout(barmode="group", height=350, margin=dict(l=20, r=20, t=20, b=20), plot_bgcolor="rgba(0,0,0,0)")
    st.plotly_chart(fig, use_container_width=True)
    ...
    with left:
        ...
        for sem in cur_a.get("semesters", []):
            with st.expander(f"Sem {sem.get('semester_number','')}"):
                for c in sem.get("courses", []):
                    st.write(f"• **{c.get('course_name','')}** ({c.get('credits',4)} cr)")
    with right:
...
def _page_chat():
    ...
    if ctx:
        ...
        st.write(f"Asking about: **{ctx.get('curriculum_title', 'your curriculum')}**")
        ...
        with c1:
            if st.button("🗑️ Clear", key="clear_chat", use_container_width=True):
                ...
                st.rerun()
        with c2:
            if st.button("↩️ Exit Doubts", key="exit_doubts", use_container_width=True):
                st.session_state.curriculum_chat_context = None
                st.session_state.chat_history = []
                st.rerun()
    else:
        st.header("💬 CurricuForge Assistant")
        st.write("Ask about **curriculum design, education, course planning, skill development**, or learning paths.")
    ...
    if not st.session_state.chat_history:
        with st.chat_message("assistant"):
            if ctx:
                st.write(f"👋 I have your **{ctx.get('curriculum_title','')}** loaded. Ask me anything!")
            else:
                st.write("👋 Hello! I can help with curriculum design, career paths, and skill development.")

    user_input = st.chat_input("Ask about your curriculum..." if ctx else "Ask a question...")
    if user_input:
        ...
        if ctx and not extra_history:
            ...
            extra_history.insert(0, {"role": "user", "content": f"Here is my curriculum:\n\n{summary}\n\nAnswer questions about it."})
            extra_history.insert(1, {"role": "assistant", "content": "I've reviewed the curriculum. Ask me anything!"})

        with st.chat_message("assistant"):
            with st.status("Thinking...", expanded=True) as status_widget:
                st.write("🤖 Consulting AI...")
                if AI_ENGINE_AVAILABLE:
                    response, backend = chat_with_llm(user_input, extra_history, CHAT_SYSTEM_PROMPT)
                else:
                    response, backend = "❌ ai_engine.py not found.", "none"
                status_widget.update(label="✅ Ready", state="complete", expanded=False)
            st.write(response)
...
def _page_about():
    ...
    st.write("AI-powered platform for structured, semester-wise curriculum generation.")
    st.divider()
    ...
    with c1:
        with st.container(border=True):
            ...
            st.write("**Primary:** Groq (Llama 3.3 70B) with round-robin key rotation.")
            st.write("**Fallback:** OpenRouter — auto-switches when Groq is unavailable.")
            status = get_backend_status()
            st.markdown(f"{'🟢' if status['groq_available'] else '🔴'} Groq ({status['groq_keys']} keys)  ·  "
                        f"{'🟢' if status['openrouter_available'] else '🔴'} OpenRouter")

        with st.container(border=True):
            st.subheader("🎯 Who Is This For?")
            st.markdown("- **Educators** — Design or update syllabi\n- **Institutions** — Rapid prototyping\n"
...
def main():
    ...
    _pages = ["Home", "Generator", "Analytics", "History", "Compare", "Chat", "About"]
    current = st.session_state.get("current_page", "Home")
    if current not in _pages:
        current = "Home"

    page = st.radio("Navigate",
                     ["🏠 Home", "⚒️ Generator", "📊 Analytics", "🗂️ History", "🔄 Compare", "💬 Chat", "ℹ️ About"],
                     index=_pages.index(current), horizontal=True, label_visibility="collapsed")

    _map = {"🏠 Home": "Home", "⚒️ Generator": "Generator", "📊 Analytics": "Analytics",
            "🗂️ History": "History", "🔄 Compare": "Compare", "💬 Chat": "Chat", "ℹ️ About": "About"}
    ...
    {"Home": _page_home, "Generator": _page_generator, "Analytics": _page_analytics,
     "History": _page_history, "Compare": _page_compare, "Chat": _page_chat, "About": _page_about}[selected]()
